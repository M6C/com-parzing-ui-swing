...
/*    Generated:  September 9th 2009 11:41:39 AM PDT                           */
/*      Machine:  10.16.139.107                                                */
/*       Source:  Local Cache                                                  */
/*     Location:  rsrc:4:72gt2jmr                                              */
/*       Locale:  nu_ll                                                        */
...
function share_show_error(params){var dialog=new Dialog().setTitle(_tx("URL invalide")).setBody(_tx("D\u00e9sol\u00e9, URL incorrecte.")).setButtons(Dialog.OK).show();return false;}
function share_is_shareable(url){return(/(?:https?:\/\/)?\w+\.\w+/.test(url));}
function share_internal_bookmarklet(){if(/http:\/\/[^\/]*?\/posted?\.php/.test(document.location.href)){var dialog=new Dialog().setClassName('share_congratulations').setTitle(_tx("Partager")).setBody('<div class="UIMessageBox status">'+'<h2>'+_tx("Vous avez correctement install\u00e9 le marqueur de signets.")+'</h2>'+'<p class="sub_message">'+_tx("Cliquez sur un site externe \u00e0 partager avec vos amis.")+'</p>'+'</div>').setButtons(Dialog.OK).show();}else{throw 0;}}
...
function _share_onsubmit_to(ids,post_action,obj){var ids=$(ids);if(!ids.tokenizer||ids.tokenizer.count_names()==0){new ErrorDialog().showError(_tx("Ce champ est obligatoire."),_tx("Vous devez sp\u00e9cifier au moins un destinataire dans votre message."));return false;}
