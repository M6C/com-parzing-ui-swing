...
/*    Generated:  September 14th 2009 4:28:14 PM PDT                           */
/*      Machine:  10.16.139.102                                                */
/*       Source:  Local Cache                                                  */
/*     Location:  rsrc:4:ddudzaz4                                              */
/*       Locale:  nu_ll                                                        */
...
function _check_for_unsaved_forms(link){for(var form_id in _unsaved_forms_to_check_for){var form=ge(form_id);if(form&&!form.is_inactive&&form.original_state&&!are_equal(form.original_state,serialize_form(form))){var href=link.href;var submit=_find_first_submit_button(form);var buttons=[];if(submit){buttons.push({name:'save',label:_tx("Enregistrer"),handler:function(){submit.click();}});}
buttons.push({name:'dont_save',label:_tx("Ne pas enregistrer"),className:'inputaux',handler:function(){window.location.href=href;}});buttons.push(Dialog.CANCEL);new Dialog().setTitle(_tx("Changements non enregistr\u00e9s")).setBody(_tx("Certaines de vos modifications n'ont pas \u00e9t\u00e9 enregistr\u00e9es. Souhaitez-vous enregistrer vos modifications maintenant\u00a0?")).setButtons(buttons).setModal().show();return false;}}}
...
if(this.handler||this.errorHandler||this.transportErrorHandler){if(typeof(interp.transportError)!='undefined'){var r=new AsyncResponse();var errorDescription=Util.isDevelopmentEnvironment()?interp.transportError:_tx("Une erreur s'est produite. Nous tentons de la r\u00e9soudre dans les plus brefs d\u00e9lais. Vous pourrez bient\u00f4t r\u00e9essayer.");copy_properties(r,{error:1000,errorSummary:_tx("Oups !"),errorDescription:errorDescription});if(this.transportErrorHandler){dispatchErrorResponse(r,true);}else{Util.error('Something bad happened; provide a transport error handler for '+'complete details.');}
return;}
...
return this._requiresConfirmation;},_displayConfirmation:function(){var message=this._confirmationMessage||_tx("Souhaitez-vous vraiment effectuer cette action ?");var html='<div class="confirmation_message">'
+HTML(message)
+'</div>';var title=this._confirmationTitle||_tx("Confirmation requise");var button=this._confirmationButton||Dialog.CONFIRM;Dialog.createConfirmationDialog(html,title,button,this.finallyHandler.bind(this)).setHandler(this._displayConfirmationHandler.bind(this)).setCancelHandler(this._confirmationCallback.curry(false)).show();},_displayConfirmationHandler:function(){this.data.confirmed=1;this._requiresConfirmation=false;this._confirmationCallback(true);this.send();}});function AsyncResponse(payload){copy_properties(this,{error:0,errorSummary:null,errorDescription:null,onload:null,replay:false,payload:payload||null});return this;}
...
copy_properties(Dialog,{OK:{name:'ok',label:_tx("OK")},CANCEL:{name:'cancel',label:_tx("Annuler"),className:'inputaux'},CLOSE:{name:'close',label:_tx("Fermer")},NEXT:{name:'next',label:_tx("Suiv.")},SAVE:{name:'save',label:_tx("Enregistrer")},SUBMIT:{name:'submit',label:_tx("Envoyer")},CONFIRM:{name:'confirm',label:_tx("Confirmer")},DELETE:{name:'delete',label:_tx("Supprimer")}});copy_properties(Dialog,{OK_AND_CANCEL:[Dialog.OK,Dialog.CANCEL],_STANDARD_BUTTONS:[Dialog.OK,Dialog.CANCEL,Dialog.CLOSE,Dialog.SAVE,Dialog.SUBMIT,Dialog.CONFIRM,Dialog.DELETE],SHOULD_HIDE_OBJECTS:!ua.windows(),MODALITY:{DARK:'dark',WHITE:'white'},SHOULD_USE_IFRAME:ua.ie()<7||(ua.osx()&&ua.firefox()),dialogStack:null,newButton:function(name,label,className,handler){var button={name:name,label:label}
...
return false;},transitionComplete:function(){_BusyUIManager.stopLookingBusy();PageTransitions._current_uri=PageTransitions._most_recent_uri=PageTransitions._next_uri;PageTransitions.restoreScrollPosition();},_warnBeforeLeaving:function(warning_text,continuation){new Dialog().setTitle(_tx("Souhaitez-vous vraiment quitter cette page ?")).setSummary(_tx("Cliquez sur {ok} pour continuer ou sur {cancel} pour rester sur la page actuelle.",{ok:_tx("OK"),cancel:_tx("Annuler")})).setBody(htmlize(warning_text)).setButtons(Dialog.OK_AND_CANCEL).setHandler(continuation).setModal().show();},restoreScrollPosition:function(){var current_uri=PageTransitions._current_uri;var scroll_position=PageTransitions._scroll_positions[current_uri];if(scroll_position){DOMScroll.scrollTo(scroll_position,false);return;}
