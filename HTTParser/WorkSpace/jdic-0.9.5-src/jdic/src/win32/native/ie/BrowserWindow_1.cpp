...
{
    LogMsg("window.close");
	if(VARIANT_TRUE==IsChildWindow){
		//it is not our responsibility
		*Cancel = VARIANT_FALSE;
	} else {
		//Java use to resolve the situation for application close/continue issue.
		SendSocketMessage(m_InstanceID, CEVENT_WINDOW_CLOSE, "windowClose");
		*Cancel = VARIANT_TRUE;
	}
}
