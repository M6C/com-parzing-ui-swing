...
public class OldThrdParseUrl extends OldAncestorThread {

  public OldThrdParseUrl(FrmMain parent) {
    super(parent);
  }
  ...
  protected void getLink()
  {
    ...
    { // Parcoure les liens contenu dans la liste jTableLinkInclude
      ...
      if ( this.getHtml().length() > 0 ) {
        ...
        vListUrl = getListLink(this.getUrl(), this.getHtml(), vListMaskUrlInc, vListMaskUrlExc);
      }
    }
          ...
          if ( !this.getParent().isExclude(this.getUrl()) )
          {
            ...
            if ( (szUrl!=null) && (szUrl.length()>0) )
            {
              ...
              if ( (iNiveauActuel<iNiveauMax) && this.isSubLink())
              {
                ...
                Vector vList = getListLink(this.getUrl(), this.getHtml(), vListMaskUrlInc, vListMaskUrlExc);
                // Affiche la liste des liens chez le parent
                ...
                // Ajout les liens trouvés dans la page pour ne pas boucler sur les liens déjà trouvé.
        ...
        if ( ( iDeb > -1 ) && ( (iFinComment==-1) || (iFinComment>=iDebComment) ) )
        {
          ...
          if (str.length()>0)
          {
            try
            {
            	 ...
            	 * liste des liens à Inclure (ou si la liste est null)
            	 */
            	boolean bAdd = false;
              ...
              if (bAdd) {
              	/**
              	 * Ajout l'url si elle ne correspond pas à une entrée de 
              	 * la liste des liens à Exclure (ou si la liste est null)
              	 */
...
*/
  ...
  private String buildFileName (String path) {
    ...
    String ret = path + "\\";
    ret += Calendar.getInstance().getTimeInMillis() + "_";
