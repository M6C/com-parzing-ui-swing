...
import httparser.table.AncestorTableListSerial;
...
import httparser.thread.ThrdLink;
...
import org.m6c.parzing.thread.AncestorThread;
import org.m6c.parzing.thread.ThrdParse;
import org.m6c.parzing.thread.ThrdParseSequence;
import org.m6c.parzing.thread.ThrdParseText;
...
public class FrmMain extends JFrame /*implements Runnable*/ {
	...
	private static final long serialVersionUID = -4645447397817833045L;
...
//EXTERNALISE	private final static String DEFAULT_URL = "Url 'Http://' || 'File://'";
  ...
  private boolean bUseIncludeListAsMask = true;
...
//EXTERNALISE  private JButton btnNextUrl = new JButton();
  ...
  private ThrdLink runnerLink = null;
...
//EXTERNALISE  private int indexHeap = -1;
  ...
  private JButton btnAddNoContentBlock = new JButton();
  ...
  private JButton btnAddIncLinkInclude = new JButton();
  private JButton btnAddExcLinkInclude = new JButton();
  ...
  private JLabel jLabelUseIncludeListAsMask = new JLabel();
  ...
  private JCheckBox chkUseIncludeListAsMask = new JCheckBox();
  ...
  private JTextField txtMaxSubLevel = new JTextField();
  private JLabel jLabelMaxSubLevel = new JLabel();
...
//  EXTERNALISE    gridLayout1.setVgap(5);
    ...
    jPanel_Source_ToolBar_Right_Button.add(btnAddNoContentBlock, null);
    ...
    btnAddNoContentBlock.setText("Add Block No Content");
    
    btnAddNoContentBlock.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
      	btnAddNoContentBlock_actionPerformed(e);
      }
    });
    ...
    btnAddIncLinkInclude.setText("Inc");
    btnAddIncLinkInclude.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        btnBuildLinkInclude_actionPerformed(e);
      }
    });
    btnAddExcLinkInclude.setText("Exc");
    btnAddExcLinkInclude.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
      	btnBuildLinkExclude_actionPerformed(e);
      }
    });
    ...
    jLabelUseIncludeListAsMask.setText("Use Exc/Inc List As Mask :");
    ...
    chkUseIncludeListAsMask.setSelected(bUseIncludeListAsMask);
    chkUseIncludeListAsMask.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        chkUseIncludeListAsMask_actionPerformed(e);
      }
    });
    ...
    txtProxyHost.setText("web.pandore.log.intra.laposte.fr");//webproxy-rgs.telintrans.fr");

    txtMaxSubLevel.setText("0");
    jLabelMaxSubLevel.setText("Max Sub Level");

    jLabel18.setText("User :");
    txtProxyUser.setText("ckh146");//droca");

    jLabel19.setText("Password :");
    txtProxyPwd.setText("laposte+5");//d230roca");

    jLabel20.setText("Port :");
    txtProxyPort.setText("8080");//3128");
...
//EXTERNALISE    jScrollPane1.getViewport().add(editorHtml, null);
    ...
    jPanel_Option.add(jLabelUseIncludeListAsMask, new XYConstraints(212, 71, 136, 15));
    jPanel_Option.add(chkIndent, new XYConstraints(170, 102, 19, 20));
    jPanel_Option.add(chkClean, new XYConstraints(170, 86, 16, 16));
    jPanel_Option.add(chkCorrectHtml, new XYConstraints(290, 86, 16, 16));
    jPanel_Option.add(chkUseIncludeListAsMask, new XYConstraints(345, 71, 16, 16));
    ...
    jPanel_Option.add(txtTimeOutDwnUrl, new XYConstraints(174, 20, 20, 18));
    jPanel_Option.add(jLabelMaxSubLevel, new XYConstraints(370, 71, 96, 15));
    jPanel_Option.add(txtMaxSubLevel, new XYConstraints(455, 71, 20, 16));
...
//    editorHtml.setEditable(false);
    ...
    jPanel8.add(btnAddIncLinkInclude,  new XYConstraints(716, 22, 33, 20));
    jPanel8.add(btnAddExcLinkInclude,  new XYConstraints(746, 22, 33, 20));
    ...
    chkUseIncludeList.setEnabled(!bUseIncludeListAsMask);
    chkUseIncludeListAsMask.setEnabled(!bUseIncludeList);
...
//EXTERNALISE        this.editorHtml.setCaretPosition(0);
  ...
  void btnAddNoContentBlock_actionPerformed(ActionEvent e) {
  	String text = this.editorSource.getSelectedText();
    if ( text!=null && text.length() > 0 ) {
    	String name = JOptionPane.showInputDialog(this, "Name :");
      if ( name!=null && name.length() > 0 ) {
...
//EXTERNALISE  }
  ...
  void btnModify_actionPerformed(ActionEvent e) {
    if ( (this.txtTextStart.getText().length()>0)
    	
    		//&& (this.txtTextEnd.getText().length()>0) &&
...
//      this.runnerLink.interrupt();
	...
	if (getTxtMethode().equals(AncestorThread.METHODE_SEQUENCE)) {
		new ThrdParseSequence(getTxtUrl(), getHtml(), parameter, list).start();
	} else {
		new ThrdParseText(getTxtUrl(), getHtml(), parameter, list).start();
	}
  }
  ...
  protected void runGetLinkProcess()
  {
    ...
    this.runnerLink = new ThrdLink(this);
...
//EXTERNALISE  }
  ...
  {
    return org.m6c.ressource.FxHtml.cleanHtml(str);
  }
...
//EXTERNALISE  }
  ...
  public boolean isUseIncludeList() {
    return bUseIncludeList && chkUseIncludeList.isEnabled();
  }
  ...
  public boolean isUseIncludeListAsMask() {
  	return bUseIncludeListAsMask && chkUseIncludeListAsMask.isEnabled();
  }
...
//EXTERNALISE	}
  ...
  public void setShowPage(boolean bShowPage) {
	...
	public void setUseIncludeListAsMask(boolean useIncludeListAsMask) {
		bUseIncludeListAsMask = useIncludeListAsMask;
		chkUseIncludeListAsMask.setSelected(bUseIncludeListAsMask);
	}
...
//EXTERNALISE	}
  ...
  public int getMaxSubLevel() {
    return Integer.parseInt(txtMaxSubLevel.getText());
  }
  public void setMaxSubLevel(int iMaxSubLevel ) {
    txtMaxSubLevel.setText(Integer.toString(iMaxSubLevel));
  }
  ...
  void chkUseIncludeListAsMask_actionPerformed(ActionEvent e) {
    chkUseIncludeList.setEnabled(bUseIncludeListAsMask);
    bUseIncludeListAsMask = !bUseIncludeListAsMask;
  }
  ...
  void btnBuildLinkInclude_actionPerformed(ActionEvent e) {
  	btnBuildAdd_actionPerformed(e, jTableLinkInclude);
  }

  void btnBuildLinkExclude_actionPerformed(ActionEvent e) {
  	btnBuildAdd_actionPerformed(e, jTableLinkExclude);
  }

  void btnBuildAdd_actionPerformed(ActionEvent e, AncestorTableListSerial jTableLink) {
    ...
    if ( commande.length() > 0 )
    {
      ...
      int iFrom = 0;
      if (szFrom!=null && !szFrom.equals("")) {
    	  try {
    		  iFrom = Integer.parseInt(szFrom.trim());
    	  }
    	  catch (Exception ex) {
    		  ex.printStackTrace();
    	  }
      ...
      int iTo = 0;
      if (szTo!=null && !szTo.equals("")) {
    	  try {
    		  iTo = Integer.parseInt(szTo.trim());
    	  }
    	  catch (Exception ex) {
    		  ex.printStackTrace();
    	  }
